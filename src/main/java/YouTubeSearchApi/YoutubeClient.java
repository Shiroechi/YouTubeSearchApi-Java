...
public class YoutubeClient {
  ...
  public YoutubeClient() { this.YOUTUBE_BASE_URL = "https://www.youtube.com/"; }

  public List<YoutubeVideo> search(String keywords, int maxResults,
                                   String language)
      throws IOException, NoResultFoundException {
    String startFeature = "ytInitialData";
    String encodedKeywords =
        URLEncoder.encode(keywords, StandardCharsets.UTF_8.toString());
    String searchUrl =
        this.YOUTUBE_BASE_URL + "results?gl=AU&hl=" + language +
    ...
    if (!foundFeatureFlag) {
      throw new NoResultFoundException(
          "What you searched was unfortunately not found or doesn't exist. keywords: " +
          keywords);
    }

    // + startFeature.length() + 3. because we want to get rid of the
    // startFeature and " = ". And get only the Json data
    int startIndex =
        pageContent.indexOf(startFeature) + startFeature.length() + 3;

    final String startIndexToEndIndex =
        pageContent.substring(startIndex, pageContent.length() - 1);
    ...
    JsonArray contents = jsonObject.get("contents")
                             .getAsJsonObject()
    ...
    for (JsonElement videoJsonElement : contents) {
      ...
      if (videoJsonObject.has("videoRenderer")) {
        JsonObject videoData =
            videoJsonObject.get("videoRenderer").getAsJsonObject();
  ...
  public YoutubePlaylist getRecommendation(String videoId)
      throws IOException, NoResultFoundException {
    ...
    Pattern pattern =
        Pattern.compile("\"compactRadioRenderer\"(.+?)\"compactVideoRenderer\"",
    ...
    if (matcher.find()) {
      ...
      compactRadioRenderer =
          "{" + compactRadioRenderer.substring(
                    0, compactRadioRenderer.length() -
                           ",{\"compactVideoRenderer\"".length());

      Gson gson = new Gson();

      JsonObject compactRadioRendererObject =
          gson.fromJson(compactRadioRenderer, JsonObject.class)
              .get("compactRadioRenderer")
              .getAsJsonObject();
      YoutubePlaylist youtubePlaylist =
          YoutubePlaylist.parseCompactRadioRenderer(compactRadioRendererObject);

      return youtubePlaylist;
    } else {
      throw new NoResultFoundException(
          "Cannot find any recommendation for videoId: " + videoId);
    }
  }
}
